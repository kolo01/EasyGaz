...
export default function Details() {
  ...
  const { product_id, price, name, zone } = useLocalSearchParams<{
    product_id:any
    price:any
    name:any
    zone:any
  }>();
  console.log( name, price, zone, product_id)
  ...
  const onDecrease = () => {
    if (quantity > 1) {
      ...
      Alert.alert("Action impossible",`impossible d'avoir moins que ${quantity}`);
    }
    setTotal(price * quantity);
  };
    ...
    <View style={styles.container}>
      <YStack paddingHorizontal={20}>
        <View style={styles.header}>
          ...
          <Text fontSize={16} fontWeight={700} color={"#626262"}>Détails de la commande</Text>
        </View>
        {/* image du gaz et la descr. */}
          ...
          <View style={styles.Boxed}>
            ...
            />
            <View>
              <Text style={styles.secondaryTextBold}>{name}</Text>
              <Text style={styles.secondaryText}>{zone}</Text>
              <Text style={styles.coloredText}>{price}</Text>
            </View>
          </View>
        ...
        <View style={styles.Boxed2}>
          ...
          <Input padding={10} borderColor={"#F4F3F3"} borderWidth={2} onChangeText={setAddress} placeholder="2plateaux non loin de SOCOCE" />
        </View>
        <TouchableOpacity onPress={() => router.push({pathname:"/logged/paiement",params: { total:total + 1000, product_id:product_id, addresse:address, quantity:quantity },})}>
          <Button onPress={() => router.push({pathname:"/logged/paiement",params: { total:total + 1000, product_id:product_id, addresse:address, quantity:quantity },})} backgroundColor={"#1D9A94"} borderRadius={4} padding={15} color={"#fff"}><Text color={"white"} fontSize={16}>Continuer  vers le paiement</Text></Button>
        </TouchableOpacity>
      </YStack>
      <YStack>
        <Footr />
